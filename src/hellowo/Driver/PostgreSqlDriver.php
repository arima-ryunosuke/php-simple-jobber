...
class PostgreSqlDriver extends AbstractDriver
{
    ...
    {
        $jobs = $this->shareJob($this->sharedFile, $this->waittime, 256, fn($limit) => array_column($this->execute("SELECT job_id, priority FROM {$this->table} WHERE start_at <= NOW() AND error IS NULL ORDER BY priority DESC LIMIT $limit FOR UPDATE SKIP LOCKED"), null, 'job_id'));
