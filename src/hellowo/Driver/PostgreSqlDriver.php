    ...
    protected function select(): Generator
    {
        ...
        foreach ($jobs as $job_id => $job) {
            ...
            try {
                ...
                $retry = yield new Message($job_id, pg_unescape_bytea($job['contents']), $job['retry']);
                if ($retry === null) {
                    $this->execute("DELETE FROM {$this->table} WHERE job_id = $1", [$job_id]);
                }
                else {
                    $job['retry']++;
                    $this->execute("UPDATE {$this->table} SET job_data = $1, start_at = NOW() + $2 WHERE job_id = $3", [$this->encode($job), "$retry SECOND", $job_id]);
                }
                $this->unshareJob($this->sharedFile, $job_id);
                $this->execute('COMMIT');
                return;
